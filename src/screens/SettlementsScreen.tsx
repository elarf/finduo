...
export default function SettlementsScreen({ navigation }: { navigation: any }) {
  ...
  /** Navigate to Dashboard and open the entry modal pre-filled with the debt data. */
        ...
        {poolsOpen && (
          <>
            ...
            {pools.map((pool) => {
              ...
              // Commitment tracking for closed pools
              ...
              const committedCount = authMems.filter((m) => {
                const involvedDebts = poolDebts.filter(
                  (d) => d.from_user === m.user_id || d.to_user === m.user_id,
                );
                if (involvedDebts.length === 0) return true;
                return involvedDebts.every((d) =>
                  d.from_user === m.user_id ? d.from_confirmed : d.to_confirmed,
                );
              }).length;
              ...
              return (
                <View key={pool.id}>
                  <TouchableOpacity
                    style={[s.poolCard, isExpanded && s.poolCardActive]}
                    onPress={() => {
                      logUI(uiPath('settlements', 'pool_card', 'container', pool.id), 'press');
                      void expandPool(pool);
                    }}
                    ...
                    <View style={s.poolDetail} {...uiProps(uiPath('settlements', 'pool_detail', 'header'))}>
                      ...
                      <View style={s.summaryCard}>
                        <View style={s.summaryRow}>
                          ...
                          <Text style={s.summaryValue}>{txLoading ? '…' : poolTotal.toFixed(2)}</Text>
                        </View>
                        <View style={s.summaryRow}>
                          <Text style={s.summaryLabel}>Members</Text>
                          <Text style={s.summaryValue}>{poolMems.length}</Text>
                        </View>
                        <View style={s.summaryRow}>
                          <Text style={s.summaryLabel}>Per person</Text>
                          <Text style={s.summaryValue}>{txLoading ? '…' : perPerson.toFixed(2)}</Text>
                        </View>
                      </View>
                      ...
                      {/* ── Active pool: settlement preview ── */}
                      {pool.status === 'active' && (
                        <View style={{ marginTop: 14 }}>
                          ...
                          {!calculating && preTxs.length === 0 && (
                            <Text style={s.balancedText}>
                              Everyone paid their fair share — no transfers needed.
                            </Text>
                          )}
                            ...
                            <>
                              <View style={s.preTxHeader}>
                                ...
                                <Text style={s.preTxHeaderText}>
                                  {preTxs.length} transfer{preTxs.length > 1 ? 's' : ''} needed
                                </Text>
                              </View>

                              {preTxs.map((ptx, i) => (
                                <View key={i} style={s.preTxRow} {...uiProps(uiPath('settlements', 'pre_tx_row', 'container', i))}>
                                  <Text style={s.preTxFrom} numberOfLines={1}>
                                    {resolveName(ptx.fromParticipantId, poolMems, ptx.metadata.fromParticipantName ?? '?')}
                                  </Text>
                                  <Icon name="ArrowRight" size={14} color="#64748B" />
                                  <Text style={s.preTxTo} numberOfLines={1}>
                                    {resolveName(ptx.toParticipantId, poolMems, ptx.metadata.toParticipantName ?? '?')}
                                  </Text>
                                  <Text style={s.preTxAmount}>{ptx.amount.toFixed(2)}</Text>
                                </View>
                              ))}

                              {/* Only the pool creator can commit */}
                              {isCreator && (
                                <TouchableOpacity
                                  style={s.commitButton}
                                  onPress={() => void handleCommit(pool)}
                                  disabled={committing}
                              ...
                              <Text style={s.preTxHint}>
                                {isCreator
                                  ? `Committing creates ${preTxs.length} pending debt${preTxs.length > 1 ? 's' : ''} and closes the pool.`
                                  : 'Only the pool creator can commit the settlement.'
                                }
                              </Text>
                            </>
                          )}
                      ...
                      {/* ── Closed pool: commitment status ── */}
                      {pool.status === 'closed' && (
                        <View style={{ marginTop: 14 }}>
                          {/* X/Y committed banner */}
                          <View style={s.committedBanner}>
                            <Icon
                              name={allCommitted ? 'CheckCircle' : 'Clock'}
                              size={16}
                              color={allCommitted ? '#53E3A6' : '#f59e0b'}
                            />
                            <Text style={[s.committedText, { color: allCommitted ? '#53E3A6' : '#f59e0b' }]}>
                              {allCommitted
                                ? 'All members committed — ready to record'
                                : `Settlement committed by ${committedCount}/${authMems.length}`
                              }
                            </Text>
                          </View>

                          {/* Pool debts the current user can see */}
                          {poolDebts.length > 0 && (
                            <>
                          ...
                          {poolDebts.length === 0 && (
                            <Text style={s.balancedText}>No settlement debts visible for this pool.</Text>
                          )}
                        </View>
                      )}
                    </View>
